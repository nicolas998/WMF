...
from qgis.PyQt.QtWidgets import QFileDialog, QTableWidgetItem, QAbstractItemView

import os.path

from . import HydroSEDPluginUtils as HSutils
from . import HydroGetCoordinates as HSCoord
from . import HydroSEDPlots as HSplots

from . import GdalTools_utils as GdalTools_utils

FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'HydroSEDPlugin_dockwidget_base.ui'))


class HydroSEDPluginDockWidget(QtWidgets.QDockWidget, FORM_CLASS):
    ...
    def setupBasinManager(self):
        ...
        def setupLineEditButtonOpenBasinFileDialog (lineEditHolder, fileDialogHolder):
            ...
            lineEditHolder.setText (fileDialogHolder.getOpenFileName (QtWidgets.QDialog (), "Cargador de cuencas", "*","Cuenca en NetCDF4(*.nc);;","", 
        ...
        def clickEventBasin2WMF():
            ...
            for k in list(self.HSutils.DicBasinNc.keys()):
        ...
        #       Simhidro, SimSed)
            ...
            for k in list(self.HSutils.DicBasinWMF.keys()):
                self.ComboGeoMaskVar.addItem(k)
            #Actualiza Param de claibracion 
            if Simhidro:
                self.ParamNamesCombo.clear()
                for k in list(self.HSutils.DicParameters.keys()):
        ...
        def clickEventBasinVarNC2Network():
            ...
            if self.botonClicked_NC is False:
                ...
                self.ruta2Network = SelectNetworkshapeDialog(QFileDialog)[0]
                self.Tabla_Prop_NC.setSelectionMode(QAbstractItemView.SingleSelection)
                self.botonClicked_NC = False
    ...
    def setupGeomorfologia(self):
        ...
        def clickEventExportParam2Excel():
            ...
            PathExcel = setupLineEditButtonSaveFileDialog(QFileDialog)[0]
        ...
        def clickEventGeoRasterProp():
            ...
            if self.checkBoxKubota.isChecked():
                ...
                if 'h1_max' not in list(self.HSutils.DicBasinNc.keys()): 
                ...
                if 'v_coef' not in list(self.HSutils.DicBasinNc.keys()): 
                    self.iface.messageBar().pushMessage (u'Hydro-SIG:',
            ...
            if self.checkBoxRunoff.isChecked():
                ...
                if 'Manning' not in list(self.HSutils.DicBasinWMF.keys()) and 'Manning' not in list(self.HSutils.DicBasinNc.keys()):
                    self.iface.messageBar().pushMessage (u'Hydro-SIG:',
        ...
        def clickEventUpdateAcumVarMask():
            ...
            for i in list(self.HSutils.DicBasinWMF.keys()):
                self.ComboGeoMaskVar.addItem(i+'-WMF')
            for i in list(self.HSutils.DicBasinNc.keys()):
    ...
    def setupRainfall(self):
        ...
        def setupLineEditButtonOpenShapeFileDialog (lineEditHolder, fileDialogHolder):
            '''Hace que cuando se busquen shapes solo se encuetren formatos vectoriales'''
            lineEditHolder.setText (fileDialogHolder.getOpenFileName (QtWidgets.QDialog (), "", "*", "Shapefiles (*.shp);;","")[0])
            if ((os.path.exists (lineEditHolder.text ().strip ())) and (not (self.iface is None))):
                self.iface.addVectorLayer (lineEditHolder.text ().strip (), os.path.basename (lineEditHolder.text ()).strip (), "ogr")
        
        def setupLineEditButtonSaveFileDialog (lineEditHolder, fileDialogHolder):
            '''Pone la ruta elegida en el dialogo de texto para guardado'''
            lineEditHolder.setText (fileDialogHolder.getSaveFileName (QtWidgets.QDialog (), "Guardar (cargar) binario de lluvia", "*", "RainBin (*.bin);;","")[0])
    ...
    def setupSimulation(self):
        ...
        def clickEventUpdateParamMapValues():
            ...
            for name, i, eje in zip(VarNames, list(range(1,21)), Ejes):
    ...
    def setupUIInputsOutputs (self):
        ...
        self.Tabla_Prop_WMF.setSelectionMode(QAbstractItemView.SingleSelection)
        self.Tabla_Prop_WMF.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.Button_Eliminar_Desde_WMF.clicked.connect(handleClickEventButton_Eliminar_Desde_WMF)
        self.Tabla_Prop_NC.setSelectionMode(QAbstractItemView.SingleSelection)
        self.Tabla_Prop_NC.setSelectionBehavior(QAbstractItemView.SelectRows)        
